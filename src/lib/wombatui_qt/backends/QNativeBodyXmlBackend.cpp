/*
 * QNativeBodyXmlBackend.cpp
 *
 *  Created on: 15.03.2010
 *      Author: bknopp
 */

#include "QNativeBodyXmlBackend.h"


namespace wombat
{

QNativeBodyXmlBackend::QNativeBodyXmlBackend()
{
...
bool QNativeBodyXmlBackend::read(QIODevice *_device)
{
    QString errorStr;
    ...
    {
    	Logger::criticalErrorMessage(errorStr.toStdString());
        return false;
    }
...
bool QNativeBodyXmlBackend::write(QIODevice *_device)
{
...
void QNativeBodyXmlBackend::setData(const std::vector<PartInfo> &_parts,
...
void QNativeBodyXmlBackend::getData(std::vector<PartInfo> &_parts,
...
QDomElement QNativeBodyXmlBackend::elementFromPart(const PartInfo &_part)
{
...
PartInfo QNativeBodyXmlBackend::partFromElement(const QDomElement &_element) const
{
	PartInfo info;
...
QDomElement QNativeBodyXmlBackend::elementFromJoint(const JointInfo &_joint)
{
...
JointInfo QNativeBodyXmlBackend::jointFromElement(const QDomElement &_element) const
{
	JointInfo info;
	info.childs = _element.attribute("childs", "").toStdString();
...
QDomElement QNativeBodyXmlBackend::elementFromDof(const DofInfo &_dof)
{
...
DofInfo QNativeBodyXmlBackend::dofFromElement(const QDomElement &_element) const
{
	DofInfo info;
